...
def analyze_video_content(transcript_text, video_metadata, section_requirements):
    """
    Use LLM to analyze what topics a video covers
    
    Args:
        transcript_text (str): Full or truncated transcript text
        video_metadata (dict): Video title, description, etc.
        section_requirements (dict): Section data with objectives and keywords
    ...
    if not transcript_text:
        ...
        return analyze_from_metadata(video_metadata, section_requirements)
...
"""
    ...
    try:
        ...
        print(f"   ⚠️  Error analyzing video content: {str(e)}")
        return {
            "topics_covered": [],
            "main_focus": "unknown",
...
def calculate_content_coverage(video_analysis, section_requirements):
    """
    Calculate how well video content matches section requirements
    
    Args:
        video_analysis (dict): Result from analyze_video_content()
        section_requirements (dict): Section with learning objectives and keywords
...
"""
    ...
    try:
        ...
        print(f"   ⚠️  Error calculating coverage: {str(e)}")
        return {
...
def detect_redundancy(new_video_topics, existing_videos_data):
    """
    Check if new video is redundant with already-selected videos
    
    Args:
        new_video_topics (list): Topics covered by new video
        existing_videos_data (list): Already selected videos with their topics
